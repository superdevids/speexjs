...
const INVALID_LOCAL_CHARS_RE = /[<>=()\[\]\\,;:\u0040\u0022]/
const START_OR_END_DOT_RE = /^\.|\.$/
...
export class NumberSchema extends Schema<number> {
  ...
  _parse(value: unknown): number {
    ...
    for (const check of this.checks) check(value)
    // Normalize -0 to 0 (JavaScript has signed zero)
    if (value === 0) return 0
    return value
  }
}
...
function isValidEmail(value: string): boolean {
  ...
  if (IP_DOMAIN_RE.test(domainPart)) return true
  ...
  if (START_OR_END_DOT_RE.test(localPart)) return false
  if (CONSECUTIVE_DOTS_RE.test(localPart)) return false
  if (INVALID_LOCAL_CHARS_RE.test(localPart)) return false
  ...
  for (const label of domainPart.split('.')) {
    ...
    for (let i = 0; i < label.length; i++) {
      const ch = label[i]!
      if (!((ch >= 'a' && ch <= 'z') || (ch >= 'A' && ch <= 'Z') || (ch >= '0' && ch <= '9') || ch === '-') && ch.charCodeAt(0) < 128) {
        return false
      }
    }
  }
