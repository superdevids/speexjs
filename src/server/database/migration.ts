...
export class TableBlueprint {
  ...
  polygon(name: string): ColumnDefinition {
    return this.addColumn('polygon', name)
  }

  geometry(name: string): ColumnDefinition {
    return this.addColumn('geometry', name)
  }

  ip(name: string): ColumnDefinition {
    return this.addColumn('inet', name)
  }

  mac(name: string): ColumnDefinition {
    return this.addColumn('macaddr', name)
  }
  ...
  ulid(name: string): ColumnDefinition {
...
export class Migrator {
  ...
  async rollbackBatch(batch: number): Promise<void> {
    const ran = await this.getMigrationsByBatch(batch)
    const schema = new SchemaBuilder(this.connection)
    for (const migration of ran.reverse()) {
      ...
      if (def) {
        ...
        try {
          ...
          console.error(`Rollback batch failed: ${migration.name}`, err)
          throw err
        }
      }
    }
  ...
  private async getRanMigrations(): Promise<MigrationStatusRow[]> {
    const result = await this.connection.raw('SELECT name, batch, executed_at as executedAt FROM migrations ORDER BY batch ASC, name ASC')
  ...
  private async recordMigration(name: string, batch: number): Promise<void> {
    await this.connection.raw('INSERT INTO migrations (name, batch) VALUES (?, ?)', [name, batch])
  }
