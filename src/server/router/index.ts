...
export class Router {
  ...
  private resolveCacheHits = 0
  private resolveCacheMisses = 0
  ...
  private registerResourceRoutes(name: string, controller: ControllerClass, actions: ResourceActions): this {
  ...
  private createControllerHandler(controller: ControllerClass, action: string): RouteHandler {
    return async (ctx: RouteContext) => {
      ...
      if (typeof handler === 'function') {
        ...
        throw new Error(`Action ${action} not found on controller ${controller.name}`)
      }
    }
  }
  ...
  resolve(method: string, path: string): ResolvedRoute | null {
    ...
    if (cached !== undefined) {
      this.resolveCacheHits++
      return cached
    }
    this.resolveCacheMisses++
    ...
    for (const route of this.routes) {
      ...
      const paramLen = paramKeys.length
      const params: Record<string, string> = paramLen === 0 ? Router.emptyParams : {}
  ...
  static emptyParams: Record<string, string> = Object.freeze({})
